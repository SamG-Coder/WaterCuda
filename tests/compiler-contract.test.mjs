...
test('every host module parses and all application DOM hooks exist',async()=>{
 ...
 for(const file of files){const result=spawnSync(process.execPath,['--check',fileURLToPath(new URL('../src/'+file,import.meta.url))],{encoding:'utf8'});assert.equal(result.status,0,file+': '+result.stderr);}
