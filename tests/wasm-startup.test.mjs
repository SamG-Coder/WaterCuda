...
test('CPU startup bounds in-flight frames, snapshots camera state, and releases threads on handover',async t=>{
 const messages=[];let worker,painted=0,frames=0,active=true,samples=0;
 ...
 const preview=new WasmStartup({getContext:()=>({putImageData:()=>painted++})},{onFrame:()=>frames++,onError:()=>assert.fail('unexpected error'),isActive:()=>active,beforeFrame:()=>samples++});
 ...
 assert.equal(messages.length,3,'completion immediately schedules next job without RAF');assert.equal(samples,2);
 ...
 assert.equal(messages.length,3,'hidden or suspended renderer does not continue');
 active=true;preview.frame(camera,origin);assert.equal(messages.length,4,'RAF can resume suspended rendering');
 preview.stop();preview.frame(camera,origin);assert.equal(worker.stopped,true);assert.equal(messages.length,4);
