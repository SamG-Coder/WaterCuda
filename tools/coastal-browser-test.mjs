// Optional development dependency: npm install --no-save --package-lock=false playwright@1.55.1
...
const root=fileURLToPath(new URL('../',import.meta.url)).replace(/\/$/,''),reports=path.join(root,'reports');
...
let browser,page;const errors=[],tests=[],requests=[],software=process.env.CW_SOFTWARE_GPU!=='0';
try{
 ...
 browser=await chromium.launch(options);page=await browser.newPage({viewport:{width:768,height:512}});
 ...
 await page.screenshot({path:path.join(reports,'coastal-ui.png')});
 await page.setViewportSize({width:390,height:844});await page.screenshot({path:path.join(reports,'coastal-mobile-ui.png')});
 assert.deepEqual(errors,[]);
 const report={passed:true,browser:browser.version(),softwareAdapterRequested:software,adapter:gpu.adapter,checks:gpu.checks,tests,images:{coast,shore,golden,moving},kernelCount:kernels,pageErrors:errors,notMeasured:['RTX 5080 frame rate','hardware cold compilation times','physical shoreline fluid simulation']};
...
}catch(e){await page?.screenshot({path:path.join(reports,'coastal-failure.png')}).catch(()=>{});await fs.writeFile(path.join(reports,'coastal-browser-failure.json'),JSON.stringify({error:e.stack,pageErrors:errors,tests,status:await page?.locator('#status').textContent().catch(()=>null)},null,2));throw e;}
