...
void Scene::preset(const std::string& name){
 const char* names[]={"coast","aerial","water","shore","scrub","bars","reef","coral","family"};
 const float views[][5]={{1250,210,650,.52f,-.10f},{-300,1400,-300,.7f,-.34f},{1550,7,1100,.52f,.025f},{1850,25,1250,.15f,-.30f},{2810.9f,11.51f,1131.74f,0,-.20f},{3370,42,2770,-1.4f,-.23f},{4377,-7,2784,0,-.12f},{4377,-7.3f,2810.8f,0,-.18f},{4377,-6.5f,2805,0,-.23f}};
 for(int i=0;i<9;i++)if(name==names[i]){std::copy(views[i],views[i]+5,camera.begin());camera[14]=name=="family"?2:(name=="coral"?1:0);origin[0]=origin[1]=0;rebase();return;}
...
struct Options {int width=1280,height=720,frames=0,seed=884,hour=12;bool headless=false,selfTest=false,hidden=false;std::string view="coast",weather="auto",output;};
...
struct Viewer {
 ...
 void key(WPARAM key){if(key=='J'||key=='K'){float hour=scene.camera[8]>=10?std::fmod(scene.camera[8]-10+scene.camera[5]/60,24.f):12;hour=std::fmod(hour+(key=='K'?1:23),24.f);scene.camera[8]=10+std::fmod(hour-std::fmod(scene.camera[5]/60,24.f)+24,24.f);}else if(key=='T')scene.camera[15]=(float)(((int)scene.camera[15]+1)%5);else if(key=='P')paused=!paused;else if(key=='H')help=!help;else if(key=='R')scene.camera[9]=1-scene.camera[9];else if(key=='C')scene.camera[13]=1-scene.camera[13];else if(key=='N')scene.camera[10]=float((int(scene.camera[10])+1)%3);else if(key=='G')scene.origin[2]=scene.origin[2]==2147483647?0:scene.origin[2]+1;
  else if(key>= '1'&&key<='8'){const char* views[]={"coast","aerial","water","shore","scrub","bars","reef","coral","family"};scene.preset(views[key-'1']);if(key=='8')speed=2;}
