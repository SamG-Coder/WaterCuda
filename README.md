...
**Threaded WASM startup experiment:** the same 17 CUDA kernels also run on shared-memory CPU threads while WebGPU compiles, then hand over at the current camera and world time. The reusable backend is now upstream in [CUDA-WebShader](https://github.com/SamG-Coder/cuda-webshader/blob/main/docs/wasm.md); WaterCuda vendors that version instead of maintaining a separate compiler. See [build instructions, measurements and limits](experiments/webshader-wasm/README.md). This experiment uses its own local server on port 8091 for the cross-origin isolation headers required by WASM threads; it does not replace the public Pages demo.
