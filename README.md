...
Open **http://localhost:8090** in Chrome or Edge. On Windows, double-click `START.bat`. The first driver compilation can take tens of seconds. The loading screen remains visible until the first frame completes. Later loads use verified shader artifacts and the browser cache. Use localhost or HTTPS, not `file://`.
