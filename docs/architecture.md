...
Foam uses seeded integer world lattice coordinates (1 m and 4 m cells) so camera-origin changes do not restart the pattern, even at remote integer origins. Footprint filtering fades subpixel nonlinear wash detail toward mean coverage. Open-ocean foam requires elevated, steep crests rather than steep trough faces. Shore width varies with wind. This is still procedural coverage, not persistent advected foam or a shallow-water solver. Native validation checks 300 rebase samples, output bounds, trough rejection and far-field coverage.

Seabed colour and caustics now include attenuation along both the refracted sunlight path down to the bed and the refracted viewing path back to the surface. View-path scattering remains an approximation. Detailed seabed evaluation and iterative refraction fade out as two-way blue-channel transmission falls below 12%, reaching zero at 4%; bulk water attenuation remains active. The blend applies to path corrections too, avoiding a hard detail cutoff.

Terrain evaluation skips the three inland relief bands whenever their beach/shelf multiplier is exactly zero. Normal evaluation shares one island descriptor across its four taps when all taps lie in the same world cell, retaining the general lookup across cell boundaries. A frozen pre-optimization reference checks 6,000 height/normal samples across seeds, cell boundaries and coarse footprints; the established dense ray checks remain active. No terrain frequencies, ray step bounds or output resolution are reduced.

Wave mip sampling accounts for the centroid shift introduced by each 2 x 2 averaging level, including periodic wraparound. This aligns the physical coordinates before trilinear LOD blending. An analytic linear-field fixture checks every nonconstant mip, rather than comparing two implementations sharing the same sampler. Reflection shading uses a 3 x 3 current-frame filter with material/depth/normal rejection and slope-variance-dependent blending; no additional ray queries, buffers or temporal history are introduced. It softens pixel-scale breakup but is not temporal antialiasing.

Atmospheric perspective integrates an exponential height-density profile analytically along each camera or reflected terrain ray. A 350 m scale height concentrates haze near sea level, with a weaker uniform background term; the near-horizontal limit uses a stable midpoint approximation. The colour is tied to the horizon sky with a subtle low-sun forward tint. This replaces the two unrelated distance-only fog terms, adds no buffers or ray-marching pass, and remains an artistic atmosphere approximation rather than multiple scattering. Tests compare 100 ray integrals against 1,024-step numerical integration and check segment splitting and altitude falloff.

The final 20% of the 18 km query range blends into the matching sky ray to conceal the finite ocean/terrain draw boundary, especially from aerial views. A native check verifies exact sky agreement at the limit.

Final shading evaluates sky and cloud colour lazily: land skips the unused background, and water with a valid reflection skips its unused sky fallback. The terrain reflection tracer retains its existing eager sky evaluation; introducing conditional sky evaluation inside that ray-heavy kernel regressed observed GPU time. A frozen pre-change shader checks 702 packed shading pixels across materials, reflection toggles and diagnostic modes. This changes evaluation scheduling, not resolution or colour formulas.

The ocean caches seed-dependent Gaussian Fourier coefficients in a 4 MiB GPU buffer. Animation evolves those coefficients with time and wind instead of recomputing their hashes, Gaussian samples and spectral amplitudes each frame. When time, wind and seed are unchanged, the FFT and mip passes are reused; camera movement and floating-origin rebasing do not invalidate the cache. The original eager spectrum remains available for native and browser GPU equivalence checks. This selectively integrates the spectrum optimization from feat/coastal-realism without its lighting, terrain traversal or browser workflow changes.
