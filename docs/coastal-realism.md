...
CI selects Mesa Lavapipe explicitly (`CW_GPU_BACKEND=lavapipe` with `VK_DRIVER_FILES` pointing to its ICD). It asserts the reported adapter is Mesa software Vulkan and runs the same complete renderer/pixel tests, without reducing shader quality or replacing the rendering path. The runner also accepts `CW_GPU_BACKEND=hardware` or `swiftshader`; hardware measurements must identify the actual adapter.
