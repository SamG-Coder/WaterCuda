...
export class Engine{
 async init(canvas,progress){
  ...
  this.kernels={};for(const name of ['seedOcean','oceanFft','packOcean','oceanMip','tracePrimary','reflectOcean','shadeOcean'])this.kernels[name]=await this.loader.load(name);
  ...
  this.oceanCalls=this.makeOceanCalls(this.camera,this.origin,this.waves,this.spectrum,this.fftPing);
 ...
 makeOceanCalls(camera,origin,waves,spectrum,ping){return [
  [this.kernels.seedOcean.bind({C:camera,Origin:origin,Spectrum:spectrum}),[32,32,4]],
 ...
 async validate(){
  const kernel=await this.loader.load('probeWorld');const rt=this.runtime;
  ...
  const run=async (p,o)=>{rt.write(points,p);rt.write(origin,new Int32Array([...o,0]));const batch=rt.batch();for(const [call,g]of prepare)batch.dispatch(call,g);batch.dispatch(invocation,[1]).submit();await rt.idle();return rt.read(result);};
  try{
   const a=await run(data,[0,0,42]),b=await run(data,[0,0,42]);
  ...
  }finally{for(const b of [points,origin,result,waves,camera,spectrum,ping])rt.destroyBuffer(b);}
 }
}
