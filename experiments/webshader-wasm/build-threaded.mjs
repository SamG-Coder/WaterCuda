...
import {compileThreaded} from './compiler/threaded.mjs';
const source=(await Promise.all(UNITS.map(n=>readFile(new URL('../../kernels/'+n+'.cu',import.meta.url),'utf8')))).join('\n');
const kernels=await compileThreaded(source,SPECS,{outDir:fileURLToPath(new URL('./generated/',import.meta.url))});
