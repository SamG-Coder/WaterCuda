...
export class WasmWorld{
 ...
 frame(C,Origin){
  ...
  const reef=[Math.floor(C[0]/12),Math.floor(C[2]/12),...Origin.slice(0,3)].join();if(C[1]<0&&C[14]<.5&&reef!==this.reef){this.progress('Building underwater reef');this.run('cacheReef',[128,128,1]);this.reef=reef;}
